...
if(!$id_kriteria) {
	...
	$jenis_nilai = (isset($result['ada_pilihan'])) ? $result['ada_pilihan'] : '';
	$urutan_order = (isset($result['urutan_order'])) ? trim($result['urutan_order']) : '';	
}
...
if(isset($_POST['submit'])):	
	...
	$jenis_nilai = (isset($_POST['jenis_nilai'])) ? trim($_POST['jenis_nilai']) : '';
	$urutan_order = (isset($_POST['urutan_order'])) ? trim($_POST['urutan_order']) : '';	
	...
	if(empty($errors)):
		...
		if(!empty($pilihan)): foreach($pilihan as $pil):
			...
			$urutan_order = (isset($pil['urutan']) && $pil['urutan']) ? (int) trim($pil['urutan']) : '';
		...
		<div class="col col-md-8 mx-auto">
			<div class="card mb-5">
				<div class="card-header pt-2 bg-primary text-white">
					<h3> <i class="fa fa-gavel" ></i> <?php echo $judul_page; ?></h3>
					...
					<div class="card-body">
						<form action="edit-kriteria.php?id=<?php echo $id_kriteria; ?>" method="post">
							...
							<div class="sembunyikan form-group">					
								<label for="disabledSelect">Cara Penilaian</label>
							...
							?>
							<div class="table-responsive form-group list-var clearfix <?php echo $the_class; ?>">					
								<label><h3>Pilihan Variabel</h3></label>
								<table id="pilihan-var" class=" table-bordered table-striped text-center">
									...
									<tbody>
										...
										?>								
											<tr data-counter="<?php echo $ctr; ?>">
												...
												<!-- <td><a href="#" class="red del-this-row"><span class="text-danger"></span>Hapus </a></td> -->
											</tr>
											<?php $ctr++; ?>
										<?php endwhile; endif;?>
								...
								<a href="#" class="btn btn-outline-primary tambah-pilihan sembunyikan">Tambah Pilihan</a>
								</div>
							</div>
